...
"""Take a SISRS nexus file, outputs phylip-relaxed with X missing data

    arguments:
    data  -- extension should be nex
...
def main(alignment_filename, missing_str):
    ...
    SeqIO.write(datalist, path.dirname(path.abspath(alignment_filename)) + '/'+ path.basename(alignment_filename).split('.')[0] + '_m'+missing_str+'.phylip-relaxed', "phylip-relaxed")
    locfile = open(path.dirname(path.abspath(alignment_filename))+'/'+ path.basename(alignment_filename).replace('.nex','') + '_locs_m'+missing_str+'.txt', 'w')
    locfile.write("\n".join(newlocs))
    locfile.close()
